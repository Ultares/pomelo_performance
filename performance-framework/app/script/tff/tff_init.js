...
var actor = {}
actor.actorId = 'g10000001';
actor.debug = true;
//var user_token = 'g10000001'; // actor.actorId
...
tff.init({host: '54.223.187.192', port: 80}, function () {
    tff.run();
});
...
function user_main_page(asyncCallback) {
    ...
    tff.get('/g1/api/', {'method': 'user.main_page', 'user_token': actor.actorId}, function (message) {
        if (message.code == 200) {
...
function cards_set_alignment(asyncCallback) {
    ...
    tff.get('/g1/api/', {'method': 'cards.set_alignment', 'user_token': actor.actorId}, function (message) {
        if (message.code == 200) {
...
function user_npc_info(asyncCallback) {
    ...
    tff.get('/g1/api/', {'method': 'user.npc_info', 'user_token': actor.actorId}, function (message) {
        if (message.code == 200) {
...
function reward_once_index(asyncCallback) {
    ...
    tff.get('/g1/api/', {'method': 'reward.once_index', 'user_token': actor.actorId}, function (message) {
        if (message.code == 200) {
...
function equip_put_on(asyncCallback) {
    ...
    tff.get('/g1/api/', {'method': 'equip.put_on', 'user_token': actor.actorId}, function (message) {
        if (message.code == 200) {
