...
var Client = function () {
    ...
    this.caseData = {
        ...
        'previous': false,
        debug: 0,
        oauthConsumerSecret: "fb228db2ff96bfdd9b19d2610a308f9b"
    };
    this.headers = {
...
Client.prototype.httpRequest = function (options, cb) {
    ...
    self.httpPreStr = options['isHttps'] ? 'https://' : 'http://';
    if (!!options['headers']) {
    ...
    self.url = self.httpPreStr + self.host + '/' + self.path;
    ...
    request(self.requestUrl, function (error, response, body) {
        var cData,
            cookie;
        if (!error && response.statusCode == 200) {
            ...
            cData = JSON.parse(body);
            cookie = response.headers['set-cookie'].toString().match(/(SP_LOGIN=\w+%\w+%\w+)/);
            if (cookie) {
                if (self.caseData.debug) {
                    console.log(cookie[1]);
                }

                cData.cookie = cookie[1];
            }
            cb(response.statusCode, cData);
        } else {
            if (/ngcore/.test(error.toString())) {
                if (self.caseData.debug) {
