...
enum MessageHero
{
	...
	eMessageHero_GetHeroStates_C 	= 0x1217;	// 获取英雄状态
	eMessageHero_GetHeroStates_S 	= 0x1218;	// 返回英雄状态
	eMessageHero_GetFormation_C		= 0x1219;	// 获取布阵请求
	eMessageHero_GetFormation_S		= 0x121A;	// 获取布阵回应
};
...
enum ErrorcodeHero
{
	...
	eErrorcodeHero_GetRoleStatusFailed 		= 0x121701;	// 无法获取到角色状态
};
...
message MessageNewHeroNotify
{
	...
	repeated uint32 			fraghero 	= 3 [ packed = true ];	// 已经碎片化的英雄信息
};
...
// 获取英雄状态
message MessageGetHeroStatesRequest
{
	optional MessageHero 		id 			= 1 [ default = eMessageHero_GetHeroStates_C ];
	required uint32 			type 		= 2;		// 战斗类型
};
message MessageGetHeroStatesResponse
{
	optional MessageHero 		id 			= 1 [ default = eMessageHero_GetHeroStates_S ];
	required uint32 			type 		= 2;		// 战斗类型
	repeated CharState 			states 		= 3;		// 战斗状态
};

// 获取布阵信息
message MessageGetFormationRequest
{
	optional MessageHero 		id 			= 1 [ default = eMessageHero_GetFormation_C ];
};
message MessageeGetFormationResponse
{
	optional MessageHero 		id 			= 1 [ default = eMessageHero_GetFormation_S ];
	repeated uint32 			formation 	= 2 [ packed = true ];	// 阵型
};
