...
enum MessageItem
{
	...
	eMessageItem_ExchangeFragment_C = 0x132E;					// 转换英雄碎片
	eMessageItem_ExchangeFragment_S = 0x132F;					// 转换英雄碎片返回
	eMessageItem_AddGem_S			= 0x13A1;					// 因洗练增加的新的宝石位
};
...
enum ErrorcodeItem
{
	...
	eErrorcodeItem_UpgradeNotFreeEquip      = 0x130BA1;			// 非游戏内掉落装备不可进阶
	...
	eErrorcodeItem_UsePropTitleValid 		= 0x130D07;			// 对应称号还在有效期内
	eErrorcodeItem_UsePropTitleNot 			= 0x130D08;			// 没有对应称号
	...
	eErrorcodeItem_ActivateMoneyNotEnough 	= 0x131906;			// 魂玉不足，请充值
	...
	eErrorcodeItem_ShopRefreshNot 			= 0x131D04;			// 商店不可以手动刷新
	...
	eErrorcodeItem_ShopBuyVipNotEnough 		= 0x131F06;			// VIP等级不足
	...
	eErrorcodeItem_TrumpUpInvalidParam 		= 0x132806;			// 非法参数个数
	...
	eErrorcodeItem_UniversalIDInvalid 		= 0x132E01;			// 万能碎片ID非法
	eErrorcodeItem_UniversalItemNotEnough 	= 0x132E02;			// 万能碎片个数不足
	eErrorcodeItem_ExchangeBagFull 			= 0x132E03;			// 你的背包空间不足
	eErrorcodeItem_ExchangeInvalidItemID 	= 0x132E04;			// 兑换的物品不存在
	eErrorcodeItem_ExchangeHeroFragement 	= 0x132E05;			// 您只能兑换英雄碎片
}
...
message MessageDressResponse
{
	...
	optional uint32 		fromheroid  = 5;					// 装备原来属于的英雄
};
...
message MessageEnhanceRequest
{
	...
	required uint64 		entityid 	= 2;					// 装备实体id
	required uint64 		materialid 	= 3;					// 材料实体id
	required uint32 		operate 	= 4;					// 操作类型:1:强化，2:一键强化
};
...
message MessageUsePropResponse
{
	...
	required uint32 		count 		= 2;					// 使用掉的数量
	required int32 			result		= 3;					// 使用结果
};
...
message MessageExtrasResponse
{
	...
	required uint64 		entityid 	= 2;					// 装备实体id
	repeated ExtraInfo 			extras		= 3;					// 洗练
};
...
message MessageAddGemResponse
{
	optional MessageItem 	id 			= 1 [ default = eMessageItem_AddGem_S ];
	required uint64			entityid 	= 2;					// 装备id
	required uint32 		slot 		= 3;					// 当前操作的孔
	repeated uint32     	gems        = 4 [ packed = true ];  // 新增宝石(其实就是宝石孔)
};
...
message MessageActivateExtraResponse
{
	...
	required uint64 		entityid 	= 2;					// 装备实体id
	repeated ExtraInfo 		extras 		= 3;					// 洗练属性
};
...
// 转换英雄碎片
message MessageExchangeHeroFragmentRequest
{
	optional MessageItem 	id 			= 1 [ default = eMessageItem_ExchangeFragment_C ];
	required uint32 		heroid 		= 2;					// 英雄ID
	required uint32 		times 		= 3;					// 兑换次数
};
message MessageExchangeHeroFragmentResponse
{
	optional MessageItem 	id 			= 1 [ default = eMessageItem_ExchangeFragment_S ];
	required uint32 		heroid 		= 2;					// 英雄ID
	required uint32 		times 		= 3;					// 兑换次数
};
