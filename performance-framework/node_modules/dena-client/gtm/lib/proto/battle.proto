...
enum MessageBattle
{
	...
	eMessageBattle_IntoTravelLevel_C		= 0x2018;		// 远征战斗
	eMessageBattle_IntoTravelLevel_S  		= 0x2019;		// 远征战斗返回
	eMessageBattle_SimulatePK_C 			= 0x201A;		// 模拟战斗
	eMessageBattle_SimulatePK_S 			= 0x201B;		// 模拟战斗
};
...
enum ErrorcodeBattle
{
	...
	eErrorcodeBattle_LevelGradeInvalid 		= 0x200202;		// 您提交的星级非法
	...
	eErrorcodeBattle_HostingRecordIsEmpty	= 0x200A0B;		// 没有找到对方的挂机信息
	eErrorcodeBattle_HostingIsProtecting	= 0x200A0C;		// 对方挂机正在保护中...
	...
	eErrorcodeBattle_TravelNotOpened 		= 0x201801;		// 远征暂未开放
	eErrorcodeBattle_TravelIsInFight		= 0x201802;		// 角色正在战斗
	eErrorcodeBattle_TravelCompleted 		= 0x201803;		// 您的远征已经完成, 请重置
	eErrorcodeBattle_DrawTravelRewards 		= 0x201804;		// 请领完奖励后再来战斗
	eErrorcodeBattle_FormationIsEmpty 		= 0x201805;		// 您还没有选择战斗阵型
	eErrorcodeBattle_CantFoundCharStatus 	= 0x201806;		// 没有找到你上次保存的战斗数据
	eErrorcodeBattle_TravelHeroIsDead 		= 0x201807;		// 您选择的英雄血量不足
	eErrorcodeBattle_TravelCopyInitFailed 	= 0x201808;		// 远征战斗初始化失败
	eErrorcodeBattle_TravelHeroInvalid 		= 0x201809;		// 远征选择的英雄非法
	eErrorcodeBattle_TravelHeroLevelLimited = 0x20180A;		// 选择的英雄等级不足
	eErrorcodeBattle_TravelOpponentDie 		= 0x20180B;		// 对手都死光了
	eErrorcodeBattle_FormationFull			= 0x20180C;		// 阵型满了
	eErrorcodeBattle_HeroRepeated 			= 0x20180D;		// 重复站位

	eErrorcodeBattle_SimulatePKIsFighting	= 0x201A01;		// 您正在战斗中
	eErrorcodeBattle_SimulatePKInnerError	= 0x201A02;		// 内部出错
};
...
message MessageBattleResultRequest
{
	message AttacckInfo
	{
		required uint32 	roleid 			= 1;					// 英雄ID
		required uint32 	attackstatus 	= 2;					// 攻击类型
	};
	message ShotOrder
	{
		required uint32 			index 			= 1;	// 出手索引
		repeated AttacckInfo 		orderlist 		= 2;	// 出手序列
	};

	optional MessageBattle 			id 		= 1 [ default = eMessageBattle_Result_C ];
	required uint32 				levelid = 2;	// 关卡ID
	required BattleResult 			result 	= 3;	// 结果
	optional uint32 				grade 	= 4;	// 评级
	repeated ShotOrder 				orders 	= 5;	// 攻击序列
};
message MessageBattleResultResponse
{
	...
	optional uint32 				type 		= 6;	// 战斗类型
	repeated CharState 				selfstate	= 7;	// 自身状态
	repeated CharState 				otherstate 	= 8;	// 对方状态
};
...
// 远征战斗
message MessageIntoTravelLevelRequest
{
	optional MessageBattle 			id 			= 1 [ default = eMessageBattle_IntoTravelLevel_C ];
	repeated uint32 				formation	= 2;				// 选择的阵型
};
message MessageIntoTravelLevelResponse
{
	optional MessageBattle 			id 			= 1 [ default = eMessageBattle_IntoTravelLevel_S ];
	required uint32 				seed1 		= 2;					// 种子1
	required uint32 				seed2 		= 3;					// 种子2
	required uint32 				seed3 		= 4;					// 种子3
	required uint32 				seed4 		= 5;					// 种子4
...
// 模拟挑战
message MessageSimulatePKRequest
{
	optional MessageBattle 			id 			= 1 [ default = eMessageBattle_SimulatePK_C ];
	required uint32 				levelid 	= 2;				// 关卡ID
	optional uint64 				left 		= 3;				// 攻方角色ID
	optional uint64 				right 		= 4;				// 守方角色ID
};
message MessageSimulatePKResponse
{
	optional MessageBattle 			id 			= 1 [ default = eMessageBattle_SimulatePK_S ];
	required uint32 				seed1 		= 2;	// 种子1
	required uint32 				seed2 		= 3;	// 种子2
	required uint32 				seed3 		= 4;	// 种子3
	required uint32 				seed4 		= 5;	// 种子4
	required uint32 				seed5 		= 6;	// 种子5
	required uint32 				seed6 		= 7;	// 种子6
	required uint64 				leftid 		= 8;	// 攻方角色ID
	required uint64 				rightid 	= 9;	// 守方角色ID
	required CombatGroup 			self 		= 10;	// 自己的军团信息
	optional CombatGroup 			other 		= 11;	// 对手的军团信息
};
