...
enum MessageLevel
{
	...
	eMessageLevel_GetTravelInfo_C 			= 0x140B;	// 获取远征信息
	eMessageLevel_GetTravelInfo_S 			= 0x140C;	// 返回远征信息
	eMessageLevel_DrawTravelRewards_C		= 0x140D;	// 领取远征奖励
	eMessageLevel_DrawTravelRewards_S 		= 0x140E;	// 返回远征奖励
	eMessageLevel_ResetTravel_C 			= 0x140F;	// 重置远征
	eMessageLevel_ResetTravel_S 			= 0x1410;	// 重置远征信息
};
...
enum ErrorcodeLevel
{
	...
	eErrorcodeLevel_TravelNotOpened 		= 0x140B01;	// 远征系统暂未开放
	eErrorcodeLevel_TravelOpponentInvalid	= 0x140B02;	// 远征对手暂时没有找到哦
	eErrorcodeLevel_TravelNotOpened2		= 0x140D01;	// 远征系统暂未开放
	eErrorcodeLevel_TravelCompleted 		= 0x140D02;	// 您的远征已经完成, 请重置
	eErrorcodeLevel_TravelSlotIsBattle 		= 0x141D03;	// 您未通关暂时不能领取宝箱
	eErrorcodeLevel_TravelBagIsFull 		= 0x140D04;	// 您的背包已经满了

	eErrorcodeLevel_TravelNotOpened3 		= 0x140F01;	// 远征系统暂未开放
	eErrorcodeLevel_TravelResetLimited 		= 0x140F02;	// 您今天的重置次数用完了
};
...
// 获取远征信息
message MessageGetTravelInfoRequest
{
	optional MessageLevel 	id 		= 1 [ default = eMessageLevel_GetTravelInfo_C ];
};
message MessageGetTravelInfoResponse
{
	message HeroInfo
	{
		...
		required uint32 		position 	= 5;	// 站位
		required uint32 		maxhp 		= 6;	// 最大血量
		required CharState 		herostate 	= 7;	// 属性
	};
	...
	optional MessageLevel 	id 		= 1 [ default = eMessageLevel_GetTravelInfo_S ];
	required uint32 		slot 	= 2;		// 当前的格子
	required uint32 		times 	= 3;		// 今天重置次数
	required Opponent 		opponent= 4;		// 对手信息
};

// 领取奖励
message MessageDrawTravelRewardsRequest
{
	optional MessageLevel 	id 		= 1 [ default = eMessageLevel_DrawTravelRewards_C ];
};
message MessageDrawTravelRewardsResponse
{
	optional MessageLevel 	id 		= 1 [ default = eMessageLevel_DrawTravelRewards_S ];
	repeated Resource 		rewards = 2;		// 奖励
};

// 重置远征
message MessageResetTravelRequest
{
	optional MessageLevel 	id 		= 1 [ default = eMessageLevel_ResetTravel_C ];
};
message MessageResetTravelResponse
{
	optional MessageLevel 	id 		= 1 [ default = eMessageLevel_ResetTravel_S ];
	required uint32 		times 	= 2;		// 重置次数
};
