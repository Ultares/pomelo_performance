...
message GuildMember {
  ...
  optional uint32 activity_point = 10;              // 活跃度
}
...
// 红包数据
message GuildGiftReceiveRecord {
  optional uint64 timestamp = 1;                    // 啥时候收的
  optional string name = 2;                         // 发红包者的名字
...
message GuildGiftReceiver {
  ...
  optional uint64 timestamp = 5;                    // 啥时候收的
}
...
// 公会成员动态信息
enum EGuildMessageType {
  GUILD_MSG_INVALID = 0;
  GUILD_MSG_GUILD_NAME = 1;                         // 用到 guild_basic_info.name
  GUILD_MSG_GUILD_ICON = 2;                         // 用到 guild_basic_info.icon
  GUILD_MSG_GUILD_LEVEL = 3;                        // 用到 guild_basic_info.level
  GUILD_MSG_MEMBER_JOIN = 10;                       // 用到 player_uid, player_name
  GUILD_MSG_MEMBER_LEAVE = 11;                      // 用到 player_uid, player_name
  GUILD_MSG_MEMBER_GET_CARD = 20;                   // 用到 player_uid, player_name, card_id, gain_reason
  GUILD_MSG_CHANGE_TITLE = 30;                      // 用到 player_uid, player_name, old_title_type, new_title_type
}
message GuildMessage {
  optional EGuildMessageType message_type = 1;      // 信息类型
  optional uint64 timestamp = 2;                    // 信息时间
  optional GuildBasicInfo guild_basic_info = 14;
  optional uint64 player_uid = 15;
  optional string player_name = 16;
  optional uint32 card_id = 18;
  optional GainReasonType gain_reason = 19;
  optional EGuildTitleType old_title_type = 20;
  optional EGuildTitleType new_title_type = 21;
}
...
message GuildInfo {
  ...
  optional uint64 wish_timestamp = 22;              // 心愿时间戳
  repeated GuildMessage guild_messages = 5;         // 成员动态信息
}
...
message PlayerGuildInfo {
  ...
  repeated GuildGiftReceiveRecord gift_receive_records = 5;        // 收红包记录
...
message S2C_GuildApplyResponse {
  ...
  optional uint32 apply_count = 4;                  // 今日申请次数
}
...
message S2C_GuildReceiveGiftResponse {
  ...
  optional GuildGiftInfo gift_info = 3;
  optional GuildGiftReceiveRecord new_receive_record = 4;
}
...
// 获取公会成员动态信息
message C2S_GuildMessageRequest {
}
message S2C_GuildMessageResponse {
  optional ErrorCode error_code = 1;
  repeated GuildMessage guild_messages = 2;         // 成员动态信息
}
