...
enum ActivityType {
  ...
  ACTIVITY_TYPE_TOTAL_GAMBLE = 11;          // 活动时间内的抽卡次数
  ACTIVITY_TYPE_RECHARGE_COUNT = 12;        // 充值次数
  ACTIVITY_TYPE_SHARE_GAME = 13;            // 分享游戏次数
  ACTIVITY_TYPE_CARD_S_PLUS_COUNT = 14;     // 拥有S+品质选手X个
  ACTIVITY_TYPE_CARD_SHOW_COUNT = 15;       // 图鉴收集 X 个选手
  ACTIVITY_TYPE_CONSUME_COIN = 16;          // 累计消耗金币
  ACTIVITY_TYPE_CONSUME_VIGOR = 17;         // 累计消耗体力
  ACTIVITY_TYPE_RECHARGE_LEVEL = 21;        // 重置>=配置数值后等级达到X级
  ACTIVITY_TYPE_EXCHANGE = 22;              // 兑换道具
  ACTIVITY_TYPE_CARD_FRAGMENT = 23;         // 获得X个任意选手碎片
  ACTIVITY_TYPE_CARD_FRAGMENT_S = 24;       // 获得X个S级选手碎片
  ACTIVITY_TYPE_CARD_FRAGMENT_S_PLUS = 25;  // 获得X个S+级选手碎片
  ...
  ACTIVITY_TYPE_CARD_LEVEL_X = 115;         // 指定ID选手升至X级（选手ID/等级）
  ACTIVITY_TYPE_STAR_LEVEL_UP_X = 116;      // 指定ID选手升至X星（选手ID/星级）
  ACTIVITY_TYPE_AWAKEN_COUNT_X = 117;       // 指定ID选手升至X阶（选手ID/阶级）
}
...
message ActivityRecord {
  ...
  optional int32 counter = 4;               // 兑换次数（活动类型为22时这个字段才有效）
}

message ActivityGroup {
  optional int32 event_id = 1;              // 活动ID
  repeated ActivityRecord records = 2;      // 这个活动里的条目
  optional uint64 login_timestamp = 3;      // 活动类型为4时这个字段才有效
  optional uint32 login_progress = 4;       // 活动类型为4时这个字段才有效
}
...
message S2C_ActivityInfoResponse {
  ...
  optional uint64 server_open_time = 2;     // 服务器开服时间
}
