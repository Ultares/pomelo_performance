...
Protocol.encode = function (EProtoId, EProtoBody) {
    ...
    byteBuffer.ushort(byteLength + 4);
...
Protocol.decode = function (msg) {
    ...
    if (msg.length === 4) {
        var unpackArray = byteBuffer.ushort().ushort().unpack();
        package.EProtoLen = unpackArray.shift();
        package.EProtoId = unpackArray.pop();
        package.EProtoBody = new Buffer(0);
    } else if (msg.length > 4) {
        var unpackArray = byteBuffer.ushort().ushort().byteArray(null, msg.length - 4).unpack();
        package.EProtoLen = unpackArray.shift();
...
Esport.prototype.connect = function (params, cb) {
    ...
    this.socket.on('error', function (error) {
        self.log('tcp-socket error code: ' + error.code, 'error');
    });
