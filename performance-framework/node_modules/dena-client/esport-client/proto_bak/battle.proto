...
// refresh_id的格式：40x2y  其中x=1蓝方，x=2红方；y=1,2,3,4,5
message FightTarget {
  optional uint32 refresh_id = 1;
  optional float hitpoint = 2;                      // 血量变成了多少
  repeated uint32 add_bufflist = 3;                 // 新增buff或状态
  repeated uint32 removed_bufflist = 4;             // 去掉了的buff
}

message CardStatus {
  optional uint32 refresh_id = 1;
  repeated uint32 removed_bufflist = 2;             // 小回合结束时去掉了的buff
}

message AttackRound {
  optional uint32 attacker = 1;                     // refresh_id
  optional uint32 skill_id = 2;                     // 不用乘100
  repeated FightTarget targets = 3;
}

message FightRound {
  optional uint32 fight_round_id = 1;
  repeated AttackRound attacks = 2;
  repeated CardStatus card_status = 3;
}
...
message S2C_BattleTacticOptionResponse {
  ...
  repeated TacticOption tactic_options = 2;
...
message S2C_BattleTacticResponse {
  optional ErrorCode error_code = 1;
  repeated BattleRound new_rounds = 2;
...
enum EDragonState {
  ...
  DRAGON_STATE_ENTER = 1;                           // 通知客户端进入打龙场景
  DRAGON_STATE_START = 2;                           // 通知客户端打龙逻辑开始：is_big_dragon
...
enum ETowerState {
  ...
  TOWER_STATE_ENTER = 1;                            // 通知客户端进入推塔场景
  TOWER_STATE_START = 2;                            // 通知客户端推塔逻辑开始（塔开始掉血）：camp, tower_total_hp, tower_damage_speed
