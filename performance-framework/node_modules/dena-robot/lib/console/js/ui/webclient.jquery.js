...
function jsonToTable(json) {
    ...
    for (var i in json) {
        var ele = json[i];
        //console.log('/dena-robot/lib/console/js/ui/webclient.jquery.js ele====>',ele)
        txt += "<tr><td class=label>" + ele.key + "</td><td>" + ele.max + "</td><td>" + ele.min + "</td><td>" + ele.l1 + "</td><td>" + ele.l2 + "</td><td>" + ele.l3 + "</td><td>" + Math.round(ele.avg) + "</td><td>" + Math.round(ele.qps) + "</td><td>" + ele.num + "</td></tr>";
    }
    return txt;
};

var updateIncrData = function (sincrData) {
    //console.log("/dena-robot/lib/console/js/ui/webclient.jquery.js sincrData====>", sincrData)
    var incrData = {};
...
var updateTimesData = function (agent, user, stimeData) {
    //console.log('/dena-robot/lib/console/js/ui/webclient.jquery.js stimeData====>',JSON.stringify(stimeData))
    var conns = agent;
    var act = {};
    var summary = {};
    ...
    for (var key in act) {
        for (var agent  in stimeData) {
            ...
            if (!exist) {
                //console.log('single:[%s] and exist:[%s]',JSON.stringify(single),JSON.stringify(exist))
                summary[key] = {key: key, min: single.min, max: single.max, avg: single.avg, num: single.num};
                summary[key].l1 = single.l1;
                summary[key].l2 = single.l2;
                summary[key].l3 = single.l3;
                ...
                exist.l1 = single.l1 + exist.l1;
                exist.l2 = single.l2 + exist.l2;
                exist.l3 = single.l3 + exist.l3;
    ...
    var sortSum = _.sortBy(summary, function (ele) {
        return -1 * (ele.num);
    });
    ...
    for (var index in sortSum) {
        ...
        var key = ex.key;
        var columns = [key, 'avg', 'min', 'max','l1', 'l2', 'l3', 'qps'];
        var grow = gdata[key] || [];
        var last = grow[grow.length - 1];
        if (!last || last[0] !== ex.num) {
            grow.push([ex.num, ex.avg, ex.min, ex.max,ex.l1, ex.l2, ex.l3, ex.qps]);
        }
        gdata[key] = grow;
        var qpschart = {};
...
$(document).ready(function () {
    ...
    $("#run-button").click(function () {
        ...
        var script = $("#robotinput").find("option:selected").text();
